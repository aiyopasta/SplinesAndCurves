...
# Window size
scale = 0.54  # 0.54 on laptop, 0.7 on main
width, height = int(1080 * scale), int(1920 * scale)
...
window = pyglet.window.Window(width=width, height=height, caption="Pyglet 3D Proj Test", config=config)
...
cam_width = 0.1  # width of "fictitious camera sensor". Its height determined by aspect ratio of screen
...
# Projection matrix
def proj_mat():
    global focus, far_clip, width, height, cam_width
    aspect = height / width
    wby2, hby2 = cam_width / 2., cam_width * aspect / 2.
